    ...
    while (!connection_closed) {
        ...
        if (buffer[0] == '/') {
            if (BUF_MATCHCMD("quit")) {
                ...
                connection_closed = true;
                pthread_cancel(pt_listen);
            } else if (BUF_MATCHCMD("w")) {
                char other_name[MAX_NAME_LEN+1];
                int whisper_offset;
                sscanf(buffer, "/w %s %n", other_name, &whisper_offset);

                for (size_t i = 0; i < MAX_CONCURRENT_CLIENTS; i++) {
                    if (clients[i].fd && !strncasecmp(clients[i].name, other_name, MAX_NAME_LEN)) {
                        // log whisper, print to self, print to target
                        pthread_mutex_lock(&write_mutex);
                        if (!strcmp(clptr->name, other_name)) {
                            printf("%s (whispered to self): %s\n", clptr->name, buffer+whisper_offset);
                            dprintf(client_fd, "(whispered to self): %s", buffer+whisper_offset);
                        } else {
                            printf("%s (whispered to %s): %s\n", clptr->name, other_name, buffer+whisper_offset);
                            dprintf(client_fd, "(whispered to %s): %s", other_name, buffer+whisper_offset);
                            dprintf(clients[i].fd, "%s (whispers): %s", clptr->name, buffer+whisper_offset);
                        }
                        pthread_mutex_unlock(&write_mutex);
                    }
                }
